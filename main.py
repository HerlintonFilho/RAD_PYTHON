...
while a == 1:
    ...
    if selecionado == '1':
        ...
        if opcao == '5':
            os.system("cls")
            whereSaldo = str(input("Digite um saldo que deseja consultar na base?\n"))
            query = cursor.execute("SELECT * FROM contas WHERE saldo => " + whereSaldo)
            queryObj = select.fetchone()
            if queryObj == None:
                print("saldo inexistente na base")
            else:
                print(query)
                banco.commit()
                folder = open("/querySaldo/saldo.txt", "w")
                folder.write(query)
                folder.close()
